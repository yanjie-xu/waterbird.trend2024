...
```
##  Family: nbinom2  ( log )
## Formula:          TotalAbundance ~ ProtectStatus * scale(Vuosi) + poly(NN, 2) +      (1 | SiteID)
## Data: mod.data
## 
##      AIC      BIC   logLik deviance df.resid 
...
## Conditional model:
##                                             Estimate Std. Error z value Pr(>|z|)    
## (Intercept)                                  1.99776    0.04585   43.57  < 2e-16 ***
## ProtectStatusProtect-Non-Helmi               1.14170    0.10224   11.17  < 2e-16 ***
## ProtectStatusProtect-Helmi                   1.65830    0.07869   21.07  < 2e-16 ***
## scale(Vuosi)                                -0.04362    0.01649   -2.65 0.008159 ** 
## poly(NN, 2)1                                 8.55393    2.31341    3.70 0.000218 ***
## poly(NN, 2)2                                 0.36470    2.48777    0.15 0.883451    
## ProtectStatusProtect-Non-Helmi:scale(Vuosi) -0.10985    0.03403   -3.23 0.001245 ** 
## ProtectStatusProtect-Helmi:scale(Vuosi)     -0.08189    0.02381   -3.44 0.000584 ***
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```
...
##               ProtectStatus 1.05 [1.03, 1.09]         1.03      0.95     [0.92, 0.97]
##                scale(Vuosi) 2.23 [2.14, 2.32]         1.49      0.45     [0.43, 0.47]
##                 poly(NN, 2) 1.05 [1.03, 1.09]         1.03      0.95     [0.92, 0.97]
##  ProtectStatus:scale(Vuosi) 2.23 [2.15, 2.33]         1.49      0.45     [0.43, 0.47]
```
...
```
##  Family: nbinom2  ( log )
## Formula:          SpeciesRichness ~ ProtectStatus * scale(Vuosi) + poly(NN, 2) +      (1 | SiteID)
## Data: mod.data
## 
##      AIC      BIC   logLik deviance df.resid 
...
## Conditional model:
##                                              Estimate Std. Error z value Pr(>|z|)    
## (Intercept)                                  1.257001   0.024091   52.18  < 2e-16 ***
## ProtectStatusProtect-Non-Helmi               0.568961   0.054060   10.52  < 2e-16 ***
## ProtectStatusProtect-Helmi                   0.869216   0.040531   21.45  < 2e-16 ***
## scale(Vuosi)                                -0.002901   0.013473   -0.22  0.82951    
## poly(NN, 2)1                                 4.374771   1.207247    3.62  0.00029 ***
## poly(NN, 2)2                                -1.223246   1.301948   -0.94  0.34745    
## ProtectStatusProtect-Non-Helmi:scale(Vuosi) -0.045653   0.027058   -1.69  0.09156 .  
## ProtectStatusProtect-Helmi:scale(Vuosi)      0.010427   0.018765    0.56  0.57845    
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```
...
## 
##                        Term  VIF   VIF 95% CI Increased SE Tolerance Tolerance 95% CI
##               ProtectStatus 1.06 [1.04, 1.10]         1.03      0.94     [0.91, 0.97]
##                scale(Vuosi) 2.39 [2.30, 2.49]         1.55      0.42     [0.40, 0.44]
##                 poly(NN, 2) 1.06 [1.03, 1.09]         1.03      0.95     [0.91, 0.97]
##  ProtectStatus:scale(Vuosi) 2.40 [2.31, 2.50]         1.55      0.42     [0.40, 0.43]
```
...
```
##  Family: nbinom2  ( log )
## Formula:          TotalAbundance ~ Hoito2020 * scale(Vuosi) + poly(NN, 2) + (1 |      SiteID)
## Data: mod.data2
## 
##      AIC      BIC   logLik deviance df.resid 
...
## 
##                    Term  VIF   VIF 95% CI Increased SE Tolerance Tolerance 95% CI
##               Hoito2020 1.07 [1.03, 1.17]         1.04      0.93     [0.86, 0.97]
##            scale(Vuosi) 2.64 [2.43, 2.88]         1.62      0.38     [0.35, 0.41]
##             poly(NN, 2) 1.07 [1.03, 1.16]         1.03      0.93     [0.86, 0.97]
##  Hoito2020:scale(Vuosi) 2.65 [2.44, 2.89]         1.63      0.38     [0.35, 0.41]
```
...
```
##  Family: nbinom2  ( log )
## Formula:          SpeciesRichness ~ Hoito2020 * scale(Vuosi) + poly(NN, 2) + (1 |      SiteID)
## Data: mod.data2
## 
##      AIC      BIC   logLik deviance df.resid 
##       NA       NA       NA       NA     1389 
...
## 
##                    Term  VIF   VIF 95% CI Increased SE Tolerance Tolerance 95% CI
##               Hoito2020 1.09 [1.05, 1.18]         1.04      0.92     [0.85, 0.96]
##            scale(Vuosi) 2.77 [2.55, 3.02]         1.66      0.36     [0.33, 0.39]
##             poly(NN, 2) 1.08 [1.04, 1.17]         1.04      0.92     [0.85, 0.96]
##  Hoito2020:scale(Vuosi) 2.83 [2.61, 3.09]         1.68      0.35     [0.32, 0.38]
```
...
## 4. Sensitivity analysis, leave one species out
### For mod2 and mod3 with total abundance

```r
species = as.vector(read.csv('single.species.csv')[,'species'])
mod.data$Hoito2020 = as.factor(mod.data$Hoito2020)

results_table = data.frame(Species = character(),
                            Model = character(),
                            Term = character(),
                            Estimate = numeric(),
                            StdError = numeric(),
...
for (i in species){
mod.data1 = mod.data[, !(colnames(mod.data) %in% i)]
mod.data1$TotalAbundance = rowSums(mod.data[,10:44], na.rm = TRUE)
mod2=glmmTMB(TotalAbundance~
                 ProtectStatus*scale(Vuosi)+
                 poly(NN,2)+
                (1|SiteID), 
...
coef_table_mod2 = summary(mod2)$coefficients$cond

for (term in rownames(coef_table_mod2)) {
    estimate = coef_table_mod2[term, "Estimate"]
    std_error = coef_table_mod2[term, "Std. Error"]
    p_value = coef_table_mod2[term, "Pr(>|z|)"]
    results_table = rbind(results_table, data.frame(Species = i,
                      Model = "mod2",
                      Term = term,
                      Estimate = estimate,
                      StdError = std_error,
...
coef_table_mod31 = summary(mod31)$coefficients$cond

for (term in rownames(coef_table_mod31)) {
    estimate = coef_table_mod31[term, "Estimate"]
    std_error = coef_table_mod31[term, "Std. Error"]
    p_value = coef_table_mod31[term, "Pr(>|z|)"]
    results_table = rbind(results_table, data.frame(Species = i,
                      Model = "mod31",
                      Term = term,
                      Estimate = estimate,
